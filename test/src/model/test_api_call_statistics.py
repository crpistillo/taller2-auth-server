...
class TestUnitsPhoto(unittest.TestCase):
    def test_median_response_time_30d(self):
        ...
        self.assertTrue((datetime.now() - timedelta(days=1)).date() in medians_by_day_dict["Jenny 1"])
        self.assertTrue((datetime.now() - timedelta(days=2)).date() in medians_by_day_dict["Jenny 1"])
        self.assertTrue((datetime.now() - timedelta(days=10)).date() in medians_by_day_dict["Jenny 1"])
        self.assertTrue((datetime.now() - timedelta(days=1)).date() in medians_by_day_dict["Jenny 2"])
        self.assertEqual(3, len(medians_by_day_dict["Jenny 1"].keys()))
        self.assertEqual(1, len(medians_by_day_dict["Jenny 2"].keys()))
        self.assertEqual(1.5, medians_by_day_dict["Jenny 1"][(datetime.now() - timedelta(days=1)).date()])
        self.assertEqual(2.0, medians_by_day_dict["Jenny 1"][(datetime.now() - timedelta(days=2)).date()])
        self.assertEqual(10.0, medians_by_day_dict["Jenny 1"][(datetime.now() - timedelta(days=10)).date()])
        self.assertEqual(1.0, medians_by_day_dict["Jenny 2"][(datetime.now() - timedelta(days=1)).date()])
