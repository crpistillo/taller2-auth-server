...
class TestUserLogin(unittest.TestCase):
    ...
    def test_login_for_non_existing_user_error(self):
        with self.app.test_client() as c:
            response = c.post('/users/login', data='{"email":"cpistillo@fi.uba.ar", "password": "carolina" }',
    ...
    def test_login_for_wrong_credentials_error(self):
        with self.app.test_client() as c:
            response = c.post('/users/register',
                              ...
                              headers={"Content-Type": "application/json"})
            self.assertEqual(response.status_code, 200)
            response = c.post('/users/login', data='{"email":"cpistillo@fi.uba.ar", "password": "caro" }',
                              headers={"Content-Type": "application/json"})
            self.assertEqual(response.status_code, 400)
    ...
    def test_register_user_and_login_for_non_existing_user_error(self):
        with self.app.test_client() as c:
            response = c.post('/users/register',
                              ...
                              headers={"Content-Type": "application/json"})
            self.assertEqual(response.status_code, 200)
            response = c.post('/users/login', data='{"email":"pistillo@fi.uba.ar", "password": "carolina" }',
                              headers={"Content-Type": "application/json"})
            self.assertEqual(response.status_code, 400)
